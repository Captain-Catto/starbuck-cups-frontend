...
class DashboardAPI {
  ...
  async getPendingConsultationsCount(token?: string): Promise<{ count: number }> {
    return this.request<{ count: number }>('/admin/consultations/pending/count', token);
  }

  // Get recent orders
  async getRecentOrders(limit: number = 10, token?: string): Promise<{ orders: RecentOrder[] }> {
    return this.request<{ orders: RecentOrder[] }>(`/admin/orders/recent?limit=${limit}`, token);
  }
