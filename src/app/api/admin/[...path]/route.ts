...
async function forwardAdminRequest(request: NextRequest, context: RouteContext) {
  try {
    const { path } = await context.params;
    const url = new URL(getApiUrl(`admin/${path.join("/")}`));
    ...
    return NextResponse.json(
      { success: false, message: "Failed to proxy admin request" },
      { status: 500 }
    );
  }
}
