...
const formatCurrency = (amount: number | undefined | null) => {
  return viCurrencyFormatter.format(amount || 0);
};
...
const OrderRow = memo(function OrderRow({
  ...
  const navigateToOrderDetail = useCallback(() => onNavigate(order.id), [order.id, onNavigate]);
  return (
    <button
      type="button"
      onClick={navigateToOrderDetail}
      className="group flex w-full items-center justify-between p-4 bg-gray-700/50 rounded-lg border border-gray-600 hover:border-gray-500 hover:bg-gray-700 transition-all cursor-pointer text-left"
    >
      <div className="flex-1">
        ...
        <p className="text-xs text-gray-400">
          {order.orderType === "product" ? order.productName : "Đơn hàng tùy chỉnh"}
        </p>
      </div>
      ...
      <div className="p-6">
        <div className="space-y-4">
          {loading ? (
            <div className="space-y-4">
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex items-center justify-between p-4 bg-gray-700/50 rounded-lg animate-pulse border border-gray-600">
                  <div className="flex-1">
            ...
            orders.map((order) => (
              <OrderRow key={order.id} order={order} onNavigate={handleNavigate} />
            ))
          ) : (
            <div className="text-center py-8 text-gray-400">
              <ShoppingCart className="size-12 mx-auto mb-2 text-gray-600" />
